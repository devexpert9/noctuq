...
export class UsersComponent implements OnInit {
  ...
  displayedColumns = [ 'image', 'name', 'email', 'phone', 'friends' ,'action'];
 dataSource : any = [];
 ...
 @ViewChild(MatPaginator, {static: true}) paginator: MatPaginator;
  ...
  constructor(private _snackBar: MatSnackBar,private modalService: NgbModal,public userService: UserService, private cdr: ChangeDetectorRef, private socket: Socket) { 
   this.getUsers(); 
  }
  ...
  open(content) 
  {
   ...
   getUsers(){
    this.isLoading = true;
    this.userService.postData({},'get_all_users').subscribe((result) => {
      console.log(result);
  ...
  confirm_delete(){
    ...
    this.userService.postData({id:this.del_id},'delete_user').subscribe((result) => {
      ...
      if(result.status == 1){
        this.socket.connect();
        this.socket.emit('deactivate_user', {userId : this.del_id});
        this.modalRef.close();
  ...
  confirm_approve()
  {
    ...
    {
      this.socket.connect();
      this.socket.emit('deactivate_user', {userId : this.status_aid});

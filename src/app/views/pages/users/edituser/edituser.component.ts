...
submit() {
  ...
  frmData.append("file", this.license_file);  
    ...
    this.userService.postData(frmData,'edit_user').subscribe((result) => { 
        if(result.status == 1){
    this.loading = false;
  ...
  uploadLicense(event,type){
   ...
   var dict = {
    ...
    this.userService.postData(dict,'reset_user_password').subscribe((result) => {   
    
        if(result.status == 1){
		this.loading = false;
		this.showSnackBar('User Password updated successfully');  
  ...
  getData(){
		this.userService.postData({id: this.id},'get_single_user').subscribe((result) => {
		this.post=result;

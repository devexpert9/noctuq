...
@Component({
  ...
  styleUrls: ['./editgenre.component.scss']
})
...
submit() {
  ...
  finalSubmit(controls){
    ...
    this.userService.postData(dict,'edit_genre_type').subscribe((result) => { 
        console.log(result);
        if(result.status == 1){
          this.loading = false;
          this.showSnackBar('Genre type has been updated successfully');  
          this.addForm.reset();
          ...
          this.loading = false;
          this.showSnackBar('Error while updating genre type,Please try after some time');
        }
    },
    err => {
...
showSnackBar(message){
  ...
  getData(){
    this.userService.postData({id: this.id},'get_genre_type').subscribe((result) => {
   console.log(result);
    this.post=result;
